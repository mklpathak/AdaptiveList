...
import co.mukulpathak.adaptivelist.AdaptiveList.Companion.VIEW_TYPE_VERTICLE
...
class AdaptiveList<T:BaseModel>(diffUtil: DiffUtil.ItemCallback<T>? = null) {
     ...
     */
    private val dataMaps = HashMap<Int, Binder<T>>()
    ...
    fun registerViewHolders(viewViewHolder: Binder<T>) {
...
//    fun getDataAdaptor () = dataBindListAdaptor
    ...
    private class DataBindListAdaptor<T:BaseModel>(
        ...
        diffUtil: DiffUtil.ItemCallback<T>) : ListAdapter<T, BinderViewHolder<T>>(
        diffUtil
    ) {
        ...
        override fun onBindViewHolder(holder: BinderViewHolder<T>, position: Int) {
    ...
    ) : BinderViewHolder<C>(itemHeaderBinding.root) {
        override fun onBind(position: Int, multiViewItem: C) {
            setData(itemHeaderBinding,multiViewItem)
        }
    }


   private class ModelHelper<B:ViewBinding,A:ViewHolderHelper<B,C>,C:BaseModel>(var holder:(parent:View)->A, var itemType: Int) : Binder<C> {
        override fun createInstance(parent: View, viewType: Int): BinderViewHolder<C> {
...
interface Binder<T:BaseModel> {
    fun createInstance(parent: View, viewType : Int) : BinderViewHolder<T>
    fun getViewType():Int
}
 ...
 */
abstract class BinderViewHolder<T:BaseModel>(itemView: View) : RecyclerView.ViewHolder(itemView) {
    abstract fun onBind(position:Int, multiViewItem: T)
}

fun <T:BaseModel> RecyclerView.listBuilder(rowsCount: Int = 4,orientation:Int =VIEW_TYPE_VERTICLE, block: AdaptiveList<T>.() -> Unit) : AdaptiveList<T>{
    val list =AdaptiveList<T>().apply {
        setUpRecyclerView(this@listBuilder,rowsCount,orientation)
    }
    block(list)
    return list
}

fun <T:BaseModel> AdaptiveList<T>.addViewHolders(vararg viewHolder:Binder<T> ){
    viewHolder.forEach {
        registerViewHolders(it)
    }
}

fun <T:BaseModel> RecyclerView.listOf(vararg viewHolder:Binder<T>) : AdaptiveList<T>{
    val list =AdaptiveList<T>().apply {
        viewHolder.forEach {
            registerViewHolders(it)
        }
        setUpRecyclerView(this@listOf,4,AdaptiveList.VIEW_TYPE_VERTICLE)
    }
    return list
}
